            ...
            messages: [
              {
                role: "system",
                content:
                  "You are an AI interviewer generating technical coding interview questions. Always tailor the question to the chosen language. The output should start with **Question:** and include code in triple backticks using the appropriate language tag (e.g., ```cpp, ```python). Do not include any explanations or markdown outside the question.",
              },
              {
                role: "user",
                content: ["Data Structures", "Algorithms"].includes(topic)
                  ? `Generate a ${difficulty} level coding interview question in ${language} on the topic ${topic}. The question should include function signature and constraints.`
                  : `Generate a ${difficulty} level interview question on the topic ${topic}. The question should be language-agnostic and focus on architecture, design, or communication.`,
              },
            ],
    ...
    app.post("/api/evaluate", async (req, res) => {
      ...
      if (!["pass", "fail"].includes(grade)) {
        if (correctness.includes("correct") && efficiency.includes("efficient")) {
          grade = "pass";
        } else {
          grade = "fail";
        }
